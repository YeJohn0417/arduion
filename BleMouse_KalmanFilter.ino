...
void loop() {
  ...
  compAngleY= 0.95 * (compAngleY + gyroYrate * dt) + 0.05 * roll; // Calculate the angle using a Complimentary filter
  compAngleX = 0.95 * (compAngleX + gyroXrate * dt) + 0.05 * pitch;
  ...
  //vx =  (compAngleX+15)/150;  
  //vy = -(compAngleY-100)/150;
  
  Serial.print("gyroX = ");   Serial.print(gyroY);
  Serial.print(", gyroY = "); Serial.print(gyroX);
